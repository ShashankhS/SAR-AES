...
def key_expansion(key):
    ...
    wkey = []
    for i in range(4):
        wkey.append((key & (filt * (mul ** i))) >> (32 * i))
    ...
    phi = 0x9e3779b9
    for i in range(8, 52):
        newk = (wkey[i - 8] ^ wkey[i - 5] ^ wkey[i - 3] ^ wkey[i - 1] ^ phi ^ i)
        wkey.append(leftRotate(newk, 11))
    ...
    for i in range(4):
        for j in wkey:
            key_schedule[i].append((j & (filt * (mul ** i))) >> (8 * i))
